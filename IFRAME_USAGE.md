...
即使宿主侧没有实现"响应 iframe 请求"的逻辑，嵌入端也能通过轮询机制自动恢复。

### 工作原理

1. **轮询启动**：页面加载后自动启动轮询（间隔 3 秒）
2. **请求数据**：定期向宿主发送 `REQUEST_HOST_DATA`
3. **响应检测**：如果宿主响应，停止轮询（避免频繁请求）
4. **页面不可见时跳过**：节省资源
...
- **轮询间隔**：3 秒（可配置）
- **请求冷却**：1 秒（避免频繁请求）
- **响应后跳过**：如果刚收到响应，跳过本次轮询
- **页面不可见时跳过**：节省资源

### 适用场景

- 宿主侧没有实现"响应 iframe 请求"的逻辑
- 刷新后宿主不响应 iframe 的请求
- 宿主侧实现不完整，需要兜底方案

### 注意事项

- 轮询会增加 postMessage 调用频率，但开销很小
- 如果宿主侧实现了响应逻辑，轮询会自动停止（避免重复）
- 轮询只在页面可见时运行，节省资源
